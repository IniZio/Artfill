<?php
	...
	*/
	...
	/*function print_page_header( $header, $caption = '' ){
		...
		$t = intval( get_query_var('tag_id') );
		 //$postval=get_post($t);
		 $args=array('tag_id' => $t);
		 $args2=query_posts($args);
		 //$postval= wp_get_post_tags( $t);
		$postnum=count($args2);
		$tex="results for";
		if( !empty($caption) ){
			echo '<div class="page-header-caption"><span class="tagcount">'.$postnum.' '.$tex.'</span> '.$caption .' </div>';
		}
		
		echo '<br> <h4 class="page-header-title">';?> <a href="<?php echo bloginfo('url'); ?>">The Shopsy Blog</a> <?php echo '</h4>';
		echo '<div class="clear"></div>';
	...
	function print_page_header( $header, $caption = '' ){
		...
		if( !empty($caption) ){
		    global $wp_query;
			$searchres = $wp_query->found_posts;
			$site_title = get_bloginfo( 'name' );
				if($searchres =='0')
				{
				echo '<div class="page-header-caption"><span class="norescount">We did not find any results for</span>' . $caption . '</div>';
				}
				else
				{
				//echo $searchres.'results for';
				echo '<div class="page-header-caption"><span class="totrescount">' . $searchres.' results for</span>'.$caption . '</div>';
	...
	function print_contact_form($item_xml){
		...
		if( $gdl_admin_translator == 'enable' ){
			...
			$gdl_submit_button = get_option(THEME_SHORT_NAME.'_translator_submit_contact_form','Sign up');
		}else{
			...
			$gdl_submit_button = __('Sign up','gdl_front_end');
		}	
