    ...
    <div class="seller-right col-md-7">
      ...
      <div class="related-listings">
		 ...
		 if($added_item_details[0]['action']=='DOD' && $added_item_details[0]['discount']!=0 && strtotime($starttime) <=  strtotime(date('Y-m-d H:i')) && strtotime($endatedeal) >=  strtotime(date('Y-m-d H:i') )) {
		  ...
		  ?>
			  ...
			  <?php if($added_item_details[0]['price']!=0){ 
						...
						<span class="cart-price" <?php echo $style;?>><?php echo $currencySymbol; echo number_format($price_tot * $currencyValue,2); ?> <span class="currencyType"><?php echo $currencyType;?></span></span>
		...
		<?php  if($this->config->item('deal_of_day')=='Yes')
		  ...
		  ?>
		<span class="cart-price" ><?php echo $currencySymbol; echo number_format($price_tot * $currencyValue,2); ?> <span class="currencyType"><?php echo $currencyType;?></span></span>
		<?php } }?>				
			  ...
			  <!-- end of deal -->
				...
				?>	
					<a data-toggle="modal" href="#ownshop_ask"><?php if($this->lang->line('ask_question') != '') { echo stripslashes($this->lang->line('ask_question')); } else echo 'Ask Question'; ?></a>
					<?php } else { ?>
					<a data-toggle="modal" href="#ask_reg"><?php if($this->lang->line('ask_question') != '') { echo stripslashes($this->lang->line('ask_question')); } else echo 'Ask Question'; ?></a>
					<?php }} else {?>
					<a href="login?action=<?php echo current_url(); ?>"><?php if($this->lang->line('ask_question') != '') { echo stripslashes($this->lang->line('ask_question')); } else echo 'Ask Question'; ?></a>
				<?php } ?>
			  </div>
				...
				<?php  if($subProduct->row()->digital_item == ''){?> 			  
				 <div class="price_left">
					...
					<?php } else if($added_item_details[0]['quantity'] == 1) { ?>
						...
						<label style="float:left; width:100%"><?php if($this->lang->line('prod_only') != '') { echo stripslashes($this->lang->line('prod_only')); } else echo 'Only 1 available'; ?></label>
...
<?php }?>				
		...
		<?php if($variations_one!='' && $variations_two!=''){ $var=2;} else if($variations_one!='' || $variations_two!=''){ $var=1; } else { $var=0; }?>
			...
			<div class="price_left">
				...
				<option value="" selected="selected"><?php if($this->lang->line('user_select') != '') { echo stripslashes($this->lang->line('user_select')); } else echo 'Select'; ?> <?php if($variations_one == "color") { if($this->lang->line('lg_color') != '') { echo stripslashes($this->lang->line('lg_color')); } else echo 'color'; }
				elseif( $variations_one == "size"){ if($this->lang->line('lg_size') != '') { echo stripslashes($this->lang->line('lg_size')); } else echo 'size'; }
				elseif( $variations_one == "weight"){ if($this->lang->line('lg_weight') != '') { echo stripslashes($this->lang->line('lg_weight')); } else echo 'weight'; }
				elseif( $variations_one == "flavor"){ if($this->lang->line('lg_flavor') != '') { echo stripslashes($this->lang->line('lg_flavor')); } else echo 'flavor'; }
				elseif( $variations_one == "length"){ if($this->lang->line('lg_length') != '') { echo stripslashes($this->lang->line('lg_length')); } else echo 'length'; }
				elseif( $variations_one == "height"){ if($this->lang->line('lg_height') != '') { echo stripslashes($this->lang->line('lg_height')); } else echo 'height'; } ?></option>
				<?php 
			  ...
			  <div id="item-overview">
				...
				<ul class="properties">
					 <li> <?php 											
									...
									echo ' ';
									if($this->lang->line('user_item_var') != '') { echo stripslashes($this->lang->line('user_item_var')); } else echo 'Item';			
								?>  
					...
					<li>
							<?php if($this->lang->line('shop_materials') != '') { echo stripslashes($this->lang->line('shop_materials')); } else echo 'Materials'; ?>: 
					...
					<?php } */?>
					 ...
					 <li><?php if($this->lang->line('feedback:') != '') { echo stripslashes($this->lang->line('feedback:')); } else echo  artfill_lg('lg_feedback', 'Feedback:'); ?> 
					...
					<?php  }*/ ?>
					 <?php if($selectedSeller_details[0]['gift_card'] == 'Yes') {?>
						...
						<li>
							<?php if($this->lang->line('giftCarg_accept') != '') { echo stripslashes($this->lang->line('giftCarg_accept')); } else echo artfill_lg('lg_giftcart accepted','Gift Card Accepted'); ?> 
						</li>
					 <?php } ?>
				</ul>
			  </div>
...
<!-- 				<?php if($added_item_details[0]['quantity'] > 0 || $subProduct->row()->digital_item!='') {?> 
					<input class="btn-primary subscribe-link alert-popupcart" id="add_to_cart" type="submit" value="<?php if ($this->lang->line('add_cart')!=''){echo $this->lang->line('add_cart');}else { echo artfill_lg('lg_add to cart','Add to Cart');}?>" <?php if($selectedSeller_details[0]['seller_id'] == $this->session->userdata['shopsy_session_user_id']){?> onclick="javascript:$('#ownproduct-link').trigger('click');"<?php } else{?> onclick="return ajax_add_cart();" <?php }?><?php if($added_item_details[0]['quantity'] <= 0) { echo 'disabled="disabled"'; } ?>  />
	   ...
	   <!-- Nav tabs -->
        <ul class="nav nav-tabs cart-tabs" role="tablist">
            <li role="presentation" class="active">
				<a href="#itemdetails" aria-controls="itemdetails" role="tab" data-toggle="tab"><?php if($this->lang->line('shop_itemdetails') != '') { echo stripslashes($this->lang->line('shop_itemdetails')); } else echo 'Item Details'; ?></a>
		    </li>
			 <?php 
			...
			<li role="presentation" id="reviewTabbar">
				<a href="#profile" aria-controls="profile" role="tab" data-toggle="tab">
					<span class="reviews">
						<div class="stars small" style="width: <?php if($rev_flg == 1){ echo $selectedSeller_details[0]['shop_ratting']*17.2; }else echo 0;?>px !important;"> </div>
					</span>(<?php if($rev_flg == 1) echo $selectedSeller_details[0]['review_count']; else echo 0; ?>)
				</a>
			 </li>
			<li role="presentation">
					<a href="#messages" aria-controls="messages" role="tab" data-toggle="tab"><?php if($this->lang->line('shop_shippingpolicy') != '') { echo stripslashes($this->lang->line('shop_shippingpolicy')); } else echo 'Shipping & Policies'; ?>
					</a>
			</li>
        </ul>
        ...
        <div class="tab-content cart-content">
            <div role="tabpanel" class="tab-pane fade in active" id="itemdetails">
             ...
             <div role="tabpanel" class="tab-pane fade" id="profile">
				<?php $i=1; foreach($productReview as $review){  if($i>4){break;}
				...
				<div class="feedback-row" id="reviews<?php echo $review['id'];?>">
				  <div class="col-md-3 feedback-reviewer"> 
					...
					<p class="feedback-reviewer"><?php if($this->lang->line('shop_reviewedby') != '') { echo stripslashes($this->lang->line('shop_reviewedby')); } else echo 'Reviewed by '; ?> <a href="view-people/<?php echo $review['userName']; ?>"><?php echo $review['fullname'].' '.$review['last_name']; ?></a></p>
				  </div>
...
line-height: 2;"><?php echo $review['title']; ?></p>
				...
				<?php $i++; }  } if($selectedSeller_details[0]['review_count'] > 4){?>
					<div class="help-bt review-cart-bt"> 
						<a href="shop-section/<?php echo $selectedSeller_details[0]['seourl']; ?>/reviews"><?php if($this->lang->line('read_all_revieews') != '') { echo stripslashes($this->lang->line('read_all_revieews')); } else echo 'Read All Reviews '; ?> (<?php echo $selectedSeller_details[0]['review_count'];?>)</a>
					</div>
				 <?php } ?>
            </div>
             ...
             <div role="tabpanel" class="tab-pane fade" id="messages">
              ...
              <h4 class="processing-time"> <?php if($this->lang->line('prod_payment') != '') { echo stripslashes($this->lang->line('prod_payment')); } else echo 'Payment Methods'; ?></h4>
			  ...
			  <?php if(($added_item_details[0]['pickup_option'] =='collection') || ($added_item_details[0]['pickup_option'] =='delivery-collecion')){?>
					<h4 class="processing-time"> <img src="images/pickup.png" width="" height="" alt="Product Pickup"><?php if($this->lang->line('prod_local_pickup') != '') { echo stripslashes($this->lang->line('prod_local_pickup')); } else echo 'This product can be collected locally'; ?></h4>
					<?php if($added_item_details[0]['pickup_option'] =='delivery-collecion'){?>
						<span style="font-weight:bold; margin-left:100px;"><?php if($this->lang->line('collection_or') != '') { echo stripslashes($this->lang->line('collection_or')); } else echo '(OR)'; ?></span>
					<?php }?>
				<?php } ?>
				<?php if($subProduct->row()->digital_item ==''){?> 
			  ...
			  <div class="shipping-information clearfix">	 
				 <?php if($subProduct->row()->ship_duration ==''){?>           
					<h4 class="processing-time"> <?php if($this->lang->line('shop_shipin') != '') { echo stripslashes($this->lang->line('shop_shipin')); } else echo 'Ready to ship in'; ?><?php echo $added_item_details[0]['ship_duration']; ?> </h4>
				  <?php } ?>
                <span class="estimate-shipping-title"><?php if($this->lang->line('shipping_costs') != '') { echo stripslashes($this->lang->line('shipping_costs')); } else echo 'Shipping Costs'; ?></span>
				    ...
				    <table class="list-table shipping">
						<thead class="tabbled">
							<tr class="column-headers">
								...
								<th class="shipping-amount"><?php if($this->lang->line('prod_another') != '') { echo stripslashes($this->lang->line('prod_another')); } else echo 'With Another Item'; ?></th>
							</tr>
						</thead>
						<tbody>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				 <div style='background:#fff;'>  
					<div class="conversation" style="width: 340px; margin-left: 191px; margin-top: 171px;">
						<div class="conversation_container">
							<h2 class="conversation_headline" style="margin: 8px;color: #9E612F;"><?php if($this->lang->line('cant_report_ur_shop') != '') { echo stripslashes($this->lang->line('cant_report_ur_shop')); } else echo 'Whoa!You can\'t report your own shop.'; ?>  </h2>
							<div class="modal-footer footer_tab_footer">
									<div class="btn-group">
											<a class="btn btn-default submit_btn" data-dismiss="modal" id="report-cancel"><?php if($this->lang->line('land_okay') != '') { echo stripslashes($this->lang->line('land_okay')); } else echo 'Okay'; ?></a>
									</div>
			...
			<div class="modal-content">
				 <div style='background:#fff;'>  
					<div class="conversation" style="width: 64%; margin-left: 191px; margin-top: 171px;">
						<div class="conversation_container">
							<h2 class="conversation_headline" style="margin: 8px;color: #9E612F;"><?php echo artfill_lg('lg_product_addeed_tocart','Product Added to cart');?> </h2>
							<div class="modal-footer footer_tab_footer">
								<div class="btn-group">
										<a class="btn btn-default submit_btn" data-dismiss="modal" ><?php echo artfill_lg('lg_continue_shop','Continue Shopping');?></a>
										<a class="btn btn-default submit_btn" href="cart"><?php echo artfill_lg('lg_go_to_checkout','Go to Checkout');?></a>
								</div>
							</div>		
						</div>
					</div>
