    ...
    <div class="main">      
        ...
        <!-- <ul class="registary_list registryhead">
			...
			if($time>1){
				if($interval->y > 0){
					if($interval->y == 1){
						echo "<span>".$interval->y. "</span>  ".artfill_lg('lg_year','Year');
					}else{
						echo "<span>".$interval->y. "</span> ".artfill_lg('lg_years','Years'); 
					}
				}
				if($interval->m > 0){
					if($interval->m == 1){
						echo "<span>".$interval->m. "</span> ".artfill_lg('lg_month','Month'); 
					}else{
						echo "<span>".$interval->m. "</span> ".artfill_lg('lg_months','Months'); 
					}
				}
				
				echo "<span>".$interval->d. "</span> ".artfill_lg('lg_day_left','days left '). "-<span>".$result->nos."</span> ".artfill_lg('lg_Items_requested','Items requested');?></h3>
			<?php }else if($time==1){
			 echo "<span> Tomorrow is your Wedding day </span> - "." <span>".$result->nos."</span>".artfill_lg('lg_Items_requested','Items requested');;?></h3>
       <?php }
	        else if($time==0)
			{
			 echo "<span>" . artfill_lg('lg_tomorrow_your_wedding','Tomorrow is your Wedding day')." </span> - "." <span>".$result->nos."</span>".artfill_lg('lg_Items_requested','Items requested');;?></h3>
       <?php
			}  
            else
            { $requestedflag=true;
            echo "<span> ". artfill_lg('lg_today_your_wedding','Today is your Wedding day')."  : ".date("d-m-Y",strtotime($d2))." </span>";?></h3> 
          <?php   }  ?>
            </li> 
          </ul> -->
		  <ul class="registary_list registryhead">
					        ...
					        ?>
						     ...
						     if($value->quantity==0){
							  echo "Un Available";
										...
										?>
										</select>  <?php echo artfill_lg('lg_requested','Requested');?>
                                        <?php }
										else
										{
										...
										?>
                                        </div>  <span class="request-num" style="color:#888"> <?php  echo $resultpurchased[0]["purchased"]; ?> <?php echo artfill_lg('lg_purchased','Purchased');?> </span>
										</div>
										<?php 
							 } ?>
                             </div> <?php 
