...
<?php if(count($shopDetail) > 0){?> <div style="text-align: center;padding: 0px 0px 12px 0px;"><h4><?php echo shopsy_lg('lg_published','Published Products');?></h4></div><?php }?>
		...
		if($this->config->item('deal_of_day')=='Yes')
		  ...
		  if($shopDetail[$i]['action']=='DOD' && $shopDetail[$i]['discount']!=0 && date('Y-m-d H:i',strtotime($starttime)) <=date('Y-m-d H:i')) {
		  ?>
		  <div class="offer-tag" >
...
<div style="text-align: center;padding: 0px 0px 12px 0px;"><h4><?php echo shopsy_lg('lg_unpublished_pro','Unpublished Products');?></h4></div>					
		...
		if($this->config->item('deal_of_day')=='Yes')
		  ...
		  if($Unpublished[$i]['action']=='DOD' && $Unpublished[$i]['discount']!=0 && date('Y-m-d H:i',strtotime($starttime)) <=date('Y-m-d H:i')) {
		  ?>
		  <div class="offer-tag" >
