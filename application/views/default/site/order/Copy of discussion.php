...
<div style="background:none repeat scroll 0 0 #FFFFFF;border-bottom:1px solid #DDDDDD;border-top:1px solid #DDDDDD;padding: 20px 0;" class="add_shop">
  ...
  <div class="main">
      ...
      <tr>
		...
		<td>
			...
			<div class="progress-bar-box" style="top:30px; width:100%;">
							...
							<div class="circle firstcircle done <?php if($Purchase->shipping_status == 'Processed'){ echo "active"; }?>">
							...
							<div class="circle done <?php if($Purchase->shipping_status == 'Shipped'){ echo "active"; }?>">
							...
							<div class="circle seccircle done <?php if($Purchase->shipping_status == 'Delivered'){ echo "active"; }?>">
	...
	<?php if($claimstatus->row()->status!='Closed') { ?> 
		<form name="dispute_attachment" id="attach_image" method="post" action="site/order/dispute_attachment_common" onsubmit="return message_validation();">
		<div class="post_textarea">
			...
			<?php //if($ViewList->row()->shipping_status == 'Processed'){ ?>
<!-- 				<input class="post_comment" type="submit" name="post_dispute" value="Cancel order"/> -->
			<?php //}?>
...
<!-- 			<select style="margin-left:0px; width:14em;" id="post_refund" class="refund-btn1"  name="post_refund">
				
				<option value="">Select</option>
				
				<?php if($ViewList->row()->received_status == 'Not received yet'){?>
					<option <?php if($ViewList->row()->received_status == 'Not received yet'){ echo "disabled selected";} ?> value="Not received yet">Not received yet</option>
					<option value="Product received">Product received</option>
				<?php }
					
				else if($ViewList->row()->received_status == 'Product received'){ ?>
					<option <?php if($ViewList->row()->received_status == 'Product received'){ echo "disabled";} ?> value="Product received">Product received</option> 
					<option <?php if($ViewList->row()->received_status == 'Need refund'){ echo "selected";} ?> value="Need refund">Request Refund</option>
					<option <?php if($ViewList->row()->received_status == 'Need reshipment'){ echo "selected";} ?> value="Need reshipment">Request Reshipment</option>
				<?php }
				
				else {?>
					<option <?php if($ViewList->row()->received_status == 'Requested Cancel'){ echo "selected";} ?> value="Requested Cancel">Requested Cancel</option>
					<option <?php if($ViewList->row()->received_status == 'Need refund'){ echo "selected";} ?> value="Need refund">Request Refund</option>
					<option <?php if($ViewList->row()->received_status == 'Need reshipment'){ echo "selected";} ?> value="Need reshipment">Request Reshipment</option>
					
					<option <?php if($ViewList->row()->received_status == 'Product returned'){ echo "selected";} ?> value="Product returned">Product Returned</option>				
					<option <?php if($ViewList->row()->received_status == 'Cancel refund'){ echo "selected";} ?> value="Cancel refund">Cancel refund</option>
					<option <?php if($ViewList->row()->received_status == 'Cancel reshipment'){ echo "selected";} ?> value="Cancel reshipment">Cancel reshipment</option>
				<?php }?>
				</select>
-->			
			...
			<?php }else{?>
				...
				<?php if($ViewList->row()->shipping_status != 'ReShipment' && $ViewList->row()->shipping_status != 'Refund' ){ ?>
				<select style="margin-left:0px; width:14em;" id="post_refund" class="cancel-bbtn1"  name="post_refund">
				<option value="">Select</option>
...
<!--				
				<?php if($ViewList->row()->shipping_status == 'Processed' || $ViewList->row()->shipping_status == 'Shipped' ||$ViewList->row()->shipping_status == 'Delivered' ){?>
					<option <?php if($ViewList->row()->shipping_status == 'Processed'){ echo "selected";} else{echo "disabled";}?> value="Processed">Processed</option>
					<option <?php if($ViewList->row()->shipping_status == 'Shipped'){ echo "selected";} else{ echo "disabled"; }?> value="Shipped">Shipped</option>
					<option <?php if($ViewList->row()->shipping_status == 'Delivered'){ echo "selected";} else{ echo "disabled"; }?> value="Delivered">Delivered</option>
				<?php }?>
				
				<?php if($ViewList->row()->received_status == 'Need refund' || $ViewList->row()->received_status == 'Need reshipment'){ ?>
					<option <?php if($ViewList->row()->shipping_status == 'Requested for Product'){ echo "selected";} ?> value="Requested for Product">Request for Product</option>
				<?php }?>
				
				<?php if($ViewList->row()->received_status == 'Product returned'){?>
					<option <?php if($ViewList->row()->shipping_status == 'ReceivedRet Product'){ echo "selected";} ?> value="ReceivedRet Product">Returned product is recieved</option>
					<option <?php if($ViewList->row()->shipping_status == 'ReShipped'){ echo "selected";} ?> value="ReShipped">Re-Shipped</option>
					<option <?php if($ViewList->row()->shipping_status == 'Refunded'){ echo "selected";} ?> value="Refunded">Refunded</option>
				<?php }?>
-->				
...
<!--				<select style="margin-left:0px; width:14em;" id="post_refund" class="post_comment"  name="post_refund">
...
<!-- 				</select> -->
				...
				<br>
				<br>
				<div class="edt" <?php if($ViewList->row()->shipping_status == 'ReShipment'){ ?> style="display:block;" <?php }else{ echo "aasasa";?> style="display:none;"  <?php }?>>
...
<!-- 				<option value="reshipment">Request Reshipment</option> -->
<!-- 				</select> -->
				<br>
				<br>
