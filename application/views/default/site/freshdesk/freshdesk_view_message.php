	 ...
	 <div class="split_prefile" >
	  ...
	  <div class="pass">
		<?php $count=count($freshdesk_view);?>
			...
			<div class="profile_field1">
					 <label><?php echo af_lg('lg_des','Description:');?> <?php  echo $freshdesk_view->helpdesk_ticket->description; ?> </label>
			</div>
				<span style="float:right;color:black;"><i><?php echo af_lg('lg_posted','Posted');?> <?php echo $msgtime; ?></i> </span>
						<?php } ?>
	 ...
	 <?php if(count($freshdesk_view->helpdesk_ticket->notes)>1){?>
		...
		<?php for($i=1;$i<count($freshdesk_view->helpdesk_ticket->notes);$i++){ ?>
				...
				<?php if($freshdesk_view->helpdesk_ticket->notes[$i]->note->source != 0)
															{$conv_name=addslashes( af_lg('lg_report','Reported by')).$freshdesk_view->helpdesk_ticket->requester_name;
															 $color="background-color:rgb(216, 240, 198);color:rgb(102, 3, 26);";
															}
																
															else{$conv_name=addslashes(af_lg('lg_reply','Replied by')).$freshdesk_view->helpdesk_ticket->responder_name; 
