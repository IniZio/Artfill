...
class User extends MY_Controller
{
    ...
    public function registerUser()
    {
        ...
        if ($this->lang->line('acc_registered') != '') {$acc_registered = stripslashes($this->lang->line('acc_registered'));} else {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($checkUser->num_rows() == 1) {
                ...
                if ($checkUser->num_rows() == 1) {
                    ...
                    if ($this->lang->line('u_r_logged') != '') {$log_in = stripslashes($this->lang->line('u_r_logged'));} else {
 ...
 */
    ...
    public function change_name($pops = '')
    {
        if ($this->checkLogin('U') == '') {
...
//         echo "<pre>";
    ...
    public function purchase_review($pType = '')
    {
     ...
     *
     */
    public function account_shipping_address()
    {
        if ($this->checkLogin('U') == '') {
            $lg_login = addslashes(shopsy_lg('lg_login', 'You must login'));
            $this->setErrorMessage('error', $lg_login);
     ...
     *
     */
    public function cart_shipping_address()
    {
        if ($this->checkLogin('U') == '') {
            $lg_login = addslashes(shopsy_lg('lg_login', 'You must login'));
            $this->setErrorMessage('error', $lg_login);
     ...
     *
     */
    public function cart_add_shipping_address()
    {
        if ($this->checkLogin('U') == '') {
            $lg_login = addslashes(shopsy_lg('lg_login', 'You must login'));
            $this->setErrorMessage('error', $lg_login);
     ...
     *
     */
    public function remove_shipping_address($id)
    {
        if ($this->checkLogin('U') == '') {
            $lg_login = addslashes(shopsy_lg('lg_login', 'You must login'));
            $this->setErrorMessage('error', $lg_login);
     ...
     *
     */
    public function remove_creadit_card($id)
    {
        if ($this->checkLogin('U') == '') {
            $lg_login = addslashes(shopsy_lg('lg_login', 'You must login'));
            $this->setErrorMessage('error', $lg_login);
     ...
     *
     */
    public function remove_order($dealCodeNumber)
    {
        if ($this->checkLogin('U') == '') {
            $lg_login = addslashes(shopsy_lg('lg_login', 'You must login'));
            $this->setErrorMessage('error', $lg_login);
     ...
     *
     */
    public function account_creditcard()
    {
        if ($this->checkLogin('U') == '') {
            $lg_login = addslashes(shopsy_lg('lg_login', 'You must login'));
            $this->setErrorMessage('error', $lg_login);
...
//die;
    ...
    public function user_addproducttolist()
    {
        if ($this->checkLogin('U') != '') {
            ...
            if (in_array($prodId, $productArr)) {
                ...
                $newproductlist = $listProduct[0]['product_id'] . $prodId . ',';
                $productCount   = $listProduct[0]['product_count'] + 1;
            }
    ...
    public function contactshop()
    {
        ...
        $datestring    = "%M %d,%Y ";
        $time          = $purchaseProducts[0]->inserttime;
    ...
    public function makeReviewBox()
    {
        ...
        if ($this->lang->line('write_review') != '') {$rew = stripslashes($this->lang->line('write_review'));} else {
            $rew = "Write a Review";
        }

        if ($this->lang->line('review_by') != '') {$rewby = stripslashes($this->lang->line('review_by'));} else {
            $rewby = "Reviewed by";
        }

        if ($this->lang->line('msg_txt') != '') {$msg = stripslashes($this->lang->line('msg_txt'));} else {
            $msg = "Message text";
        }

        if ($this->lang->line('msg_title') != '') {$msg1 = stripslashes($this->lang->line('msg_title'));} else {
            $msg1 = "Message title";
        }

        if ($this->lang->line('post_review') != '') {$post_review = stripslashes($this->lang->line('post_review'));} else {
            $post_review = "Post your Review";
        }

        if ($this->lang->line('user_cancel') != '') {$user_cancel = stripslashes($this->lang->line('user_cancel'));} else {
            $user_cancel = "Cancel";
        }
            ...
            if (!empty($userActivity)) {
                ...
                foreach ($userActivity as $actFav) {
                    ...
                    if ($actFav['activity_name'] == 'Unfavorite item' || $actFav['activity_name'] == 'favorite item') {
                        ...
                        if ($this->lang->line('user_thisitem') != '') {$content .= stripslashes($this->lang->line('user_thisitem'));} else {
                            $content .= '"this item"';
                        }
                    ...
                    <div class="activity-desc">
                        ...
                        <p class="activity-name">';
                        if ($userProfileDetails[0]['id'] != $loginCheck) {$content .= $userProfileDetails[0]['user_name'];} else {
                            if ($this->lang->line('user_you') != '') {$content .= stripslashes($this->lang->line('user_you'));} else {
                                $content .= "You";
                            }
                        }
                        $content .= ' ';
                        if ($this->lang->line('favorited') != '') {$content .= stripslashes($this->lang->line('favorited'));} else {
                            $content .= "favorited";
                        }
                        ...
                        if ($this->lang->line('user_thisshop') != '') {
                            $content .= stripslashes($this->lang->line('user_thisshop'));} else {
                            $content .= "this shop";
                        }
                    ...
                    <div class="activity-link clear">
                        <div class="activeright">
                            ...
                            <p class="line-type">';
                        if ($this->lang->line('shop') != '') {$content .= stripslashes($this->lang->line('shop'));} else {
                            $content .= "SHOP";
                        }
    ...
    public function ajax_activity_shop()
    {
        if ($this->checkLogin('U') != '') {
            ...
            if (!empty($myshopactivity)) {
                ...
                $hover   = 0;
                $s       = 0;
                $l       = 1;foreach ($myshopactivity as $actFav) {
                    if ($s < 3 && $l < 4) {
                        $cls = "small";
                        $s++;} else if ($s > 2 && $l < 4) {
                        $cls = "large";
                    ...
                    if ($actFav['activity_name'] == 'Unfavorite item' || $actFav['activity_name'] == 'favorite item') {
                        ...
                        $content .= '<div class="hoverdrop2_icon">
                                    <a href="javascript:hoverView(' . $hover . ');">
                                        <div class="hover_lists" id="hoverlist' . $hover . '">
                                            <h2>';
                        if ($this->lang->line('user_your_lists') != '') {$content .= stripslashes($this->lang->line('user_your_lists'));} else {
                            $content .= "Your Lists";
                        }
                        ...
                        $content .= '</div>
                                            <div class="new_list">
                                                <form method="post" action="site/user/add_list">
                                                    <input type="hidden" value="1" name="ddl" />
                                                    <input type="hidden" value="' . $productDetail->id . '" name="productId" />
                                                    <input type="text" placeholder="';if ($this->lang->line('user_new_list') != '') {$content .= stripslashes($this->lang->line('user_new_list'));} else {
                            $content .= 'New list';
                        }

                        $content .= '" class="list_scroll" name="list" id="creat_list_' . $hover . '" />
                                                    <input type="submit" value="';if ($this->lang->line('user_add') != '') {$content .= stripslashes($this->lang->line('user_add'));} else {
                            $content .= 'Add';
                        }
    ...
    public function ajax_people_favorite_list_itemsilove()
    {
        ...
        $i                                = 1;foreach ($userFavoriteItems as $products) {
            ...
            if (!empty($userRegistry)) {
                ...
                if ($this->lang->line('prod_wedding') != '') {$content .= stripslashes($this->lang->line('prod_wedding'));} else {
            ...
            $content .= '" class="list_scroll" name="list" id="creat_list_' . $i . '" />
                                                        <input type="submit" value="';if ($this->lang->line('user_add') != '') {$content .= stripslashes($this->lang->line('user_add'));} else {
                $content .= 'Add';
            }

            $content .= '" class="primary-button" onClick="return validate_create_list(\'' . $i . '\');" />
                                                    </form>
                                                </div>
                                        	</div>
