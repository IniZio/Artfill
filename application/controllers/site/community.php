		...
		if ($this->checkLogin('U') == ''){
			redirect('login?next=join-team/'.$teamId.'/'.$teamSEOURL);
			 ...
			 if(($checkDub->num_rows())==0){
				...
				$lg_joined_team=addslashes(af_lg('lg_joined_team','You have been joined successfully!'));
				$this->setErrorMessage('success',lg_joined_team);
				redirect('team/'.$teamId.'/'.$teamSEOURL);
			}else{
			$lg_already_joined=addslashes(af_lg('lg_already_joined','You have alredy joined in this team!'));
	...
	public function leaveTeam(){
		...
		if ($this->checkLogin('U') == ''){
			redirect('login?next=leave-team/'.$teamId.'/'.$teamSEOURL);
			 ...
			 $checkDub=$this->community_model->checkDublicatemember($Condition);
			 if(($checkDub->num_rows())==0){
			 $lg_not_a_memberin_team=addslashes(af_lg('lg_not_a_memberin_team','You are not member in this team!'));
