...
// use product_id as seourl after the insertion.
            ...
            if ($item_name == 'physical') {
                //if is it physical item we should add the attributes to the sub products table else add digital file/
                if ($UPDATE_product_Seourl != '') {
            ...
            } else if (count($this->input->post('DigiFiles')) > 1) {
                // if is it digital item
     ...
     * return Array
     */
    public function Load_ajax_products_list($cat_id)
    {
        ...
        if ($product_list->num_rows() > 0) {
            $i     = 0;
            $hover = 0;foreach ($product_list->result_array() as $products) {
                ...
                if (!empty($userRegistry)) {
                    ...
                    if ($this->lang->line('prod_wedding') != '') {$prod_list .= stripslashes($this->lang->line('prod_wedding'));} else {
                        $prod_list .= 'Wedding Registry </label>';
                    }
                ...
                $prod_list .= '</div>
						...
						<div class="new_list">
							<form action="site/user/add_list" method="post">
								...
								<input type="text" placeholder="';if ($this->lang->line('user_new_list') != '') {$prod_list .= stripslashes($this->lang->line('user_new_list'));} else {
                    $prod_list .= 'New list" class="list_scroll" name="list" id="creat_list_' . $i . '" />
								 <input type="submit" value="';
                }
                if ($this->lang->line('user_add') != '') {$prod_list .= stripslashes($this->lang->line('user_add'));} else {
                    $prod_list .= "Add";
                }

                $prod_list .= '" class="primary-button" onclick="return validate_create_list(\'' . $products['id'] . $i . '\');" />
							</form>
						</div>
 ...
 */
    ...
    /**
     * This function returns json of product details
     * @param string $seourl seorul of product
     * @return
     */
    public function get_product_details()
    {
        ...
        // Not sure what is subProduct, but it fails the controller
        // $this->data['subProduct'] = $this->product_model->get_all_details(SUBPRODUCT, array('product_id' => $this->data['preview_item_detail'][0]['id'], 'digital_item !=' => ''));
    ...
    public function load_ajax_options_div()
    {
        ...
        echo '<div class="list_inner_right list_small_width" id="variations_level_div"></div><div style="float:right;">
                            ...
                            <div class="list_inner_right list_small_width" id="attr_options_val" name="attr_options_val" style="display:none">
                                <select onchange="return alpha_check(this);" id="checked_alpha_value" class="alpha_value_one">
                                    <option value="">';if ($this->lang->line('shop_selavailableoptions') != '') {echo stripslashes($this->lang->line('shop_selavailableoptions'));} else {
            echo 'Select available options';
        }
                                	...
                                	<table width="100%" class="inner_table"  align="center" cellpadding="0" cellspacing="0" id="options_list">
                                        <tbody>
                                            <tr style="background:#E9F6FC; margin: 0px; padding: 10px; width: 100%;">
                                               <td width="25%" ><strong>';if ($this->lang->line('shop_options') != '') {echo stripslashes($this->lang->line('shop_options'));} else {
            echo 'Options';
        }
        ...
        echo '</strong></td>
                                               <td width="25%">
                                                    <span class="non-pricing-mode">
                                                        <a href="javascript:void(0)" class="enable-variations-pricing" id="price_opt" onclick="price_opt_click();">';if ($this->lang->line('shop_addpricing') != '') {echo stripslashes($this->lang->line('shop_addpricing'));} else {
            echo 'Add Pricing';
        }
        ...
        echo '</a>
                                               ...
                                               <td  width="25%" ><strong>';if ($this->lang->line('shop_instock') != '') {echo stripslashes($this->lang->line('shop_instock'));} else {
            echo 'In Stock';
        }
...
//         $this->thumbimage_resize('images/product/org-image/','images/product/thumb/','175');
        ...
        if ($this->input->get('max_price') != '' || $this->input->get('min_price') != '' || $this->input->get('price') != '') {
            $minVal = mysql_real_escape_string($this->input->get('min_price'));
            $maxVal = mysql_real_escape_string($this->input->get('max_price'));
    ...
    public function ajax_search_product()
    {
        ...
        if (!empty($productsDetail)) {
            ...
            foreach ($productsDetail as $proddetails) {
                ...
                $content .= '" class="list_scroll" name="list" id="creat_list_' . $proddetails['id'] . $i . '" />
                                                        <input type="submit" value="';if ($this->lang->line('user_add') != '') {$content .= stripslashes($this->lang->line('user_add'));} else {
                    $content .= "Add";
                }

                $content .= '" class="primary-button" onclick="return validate_create_list(\'' . $proddetails['id'] . $i . '\');" />
                                                    </form>
                                                </div>
                                        	</div>
