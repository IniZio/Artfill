...
class Seller_support extends MY_Controller {
	...
	public function create_ticket(){
		...
		if($ticket_id != ''){
		$submitted=addslashes(af_lg('lg_ur_ticket_submitted','Your ticket submitted successfully'));
			$this->setErrorMessage('success',$submitted);
			redirect('view-ticket/'.$ticket_id);
		} else {
		$not_submitted=addslashes(af_lg('lg_ur_ticket_notsubmitted','Ticket not submitted, Please try again later'));
	...
	public function freshdesk_view_message(){
		if ($this->checkLogin('U') == ''){
			...
			if($freshdesk_view->errors->error == ''){
				...
				$not_permitted_toview=addslashes(af_lg('lg_no_permission','You are not permitted to view this ticket'));
	...
	function send_ticket_note(){
		...
		$ticket_msg=addslashes(af_lg('lg_ticket_msg_submitted','Ticket message submitted successfully'));
	...
	public function freshdesk_veiw_ticket(){
		if ($this->checkLogin('U') == ''){
			...
			if($this->uri->segment(3) == ''){
				...
				$this->data['heading'] = addslashes(af_lg('lg_seller_suport_ticket','Seller Support Ticket Lists'));
