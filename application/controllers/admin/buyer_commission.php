 ...
 */
    	...
    	if ($this->checkLogin('A') == ''){
			...
			if (!$this->data['demoserverChk']){
				if ($this->checkPrivileges('admin','2') == TRUE){
					...
					$this->data['paypal_rate_value'] = $this->admin_model->getAdminSettings()->row()->paypal_rate;
					$this->data['paypal_static_value'] = $this->admin_model->getAdminSettings()->row()->paypal_static;
					$this->load->view('admin/adminsettings/edit_buyer_commission',$this->data);
				}else {
					redirect('admin');
				}
	...
	{	
		if (!$this->data['demoserverChk']){
			$mode = ADMIN_SETTINGS;
			$buyer_commission = $this->input->post('buyer_commission');
			$paypal_rate = $this->input->post('paypal_rate');
			$paypal_static = $this->input->post('paypal_static');
