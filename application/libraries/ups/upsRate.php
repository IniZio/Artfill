...
class UpsRate{
	...
	function package($params) {
		$package = $this->ups->sandwich($this->ups->templatePath.'Rates/RatingServiceSelection_Package.xml', array('{PACKAGE_DESCRIPTION}',
			'{PACKAGING_CODE}','{PACKAGE_SIZE}','{PACKAGE_EXTRAS}'), array($params['description'],$params['code'],$this->packageDimensions(array('country_code' => $params['country_code'],'length' => $params['length'], 'width' => $params['width'], 'height' => $params['height'])). $this->packageWeight(array('weight' => $params['weight'],'weight_uom'=>$params['weight_uom'])),''));
	...
	function packageDimensions($params) {
		$packageDimensions = $this->ups->sandwich($this->ups->templatePath.'Rates/RatingServiceSelection_PackageDimensions.xml', array('{PACKAGE_COUNTRY_CODE}','{PACKAGE_LENGTH}',
			'{PACKAGE_WIDTH}',
			'{PACKAGE_HEIGHT}'), array($params['country_code'],$params['length'],$params['width'],$params['height']));
	...
	function packageWeight($params) {
		...
		$packageWeight = $this->ups->sandwich($this->ups->templatePath.'Rates/RatingServiceSelection_PackageWeight.xml', array('{PACKAGE_WEIGHT}','{PACKAGE_WEIGHT_UOM}'), array($params['weight'],$params['weight_uom'])); 

		$this->packageWeightXML = $packageWeight;
		return $packageWeight;
	}
	
	function PackagePickupType($params) {
		$pickup_type = $this->ups->sandwich($this->ups->templatePath.'Rates/RatingServiceSelection_PickupType.xml', array('{PICKUP_TYPE}'), array($params['pickup_type'])); 

		$this->pickupXml = $pickup_type;
		return $pickup_type;
	}
