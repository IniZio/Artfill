...
(function(){
    function defaultEditorCallback(options){
        ...
        options.container.find('a.btn').click(function(){
			//alert(options.container.find('input').val());
    ...
    $.fn.livecsseditor.setPropertyEditor(['color','background-color'],function colorEditorCallback(options){
		//console.log(options.selector+" : "+options.value);
        var html = '<form class="form-inline"><div class="input-append color" data-color="' + options.value + '" data-color-format="rgb"><span class="add-on"><i style="background-color: ' + options.value + '"></i></span><input style="width:99px;" type="text" value="' + options.value + '" /></div></form>';
        options.container.html(html);
		 options.container.find('div.color').colorpicker().on('changeColor', function(event) {
			options.container.find('input').val(event.color.toHex());
            options.setValue(options.container.find('input').val());
            return false;      
        });
