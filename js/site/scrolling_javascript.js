(function($) {
	...
	$.fn.scrollPagination = function(options) {
		...
		return this.each(function() {
			...
			if($settings.scroll == true) $initmessage = lg_scroll_more_results;
			...
			function getData() {
				...
				$.post($settings.path, {
					...
					if(data == "") { 
					    ...
					    if($settings.page == true){
					    	$this.find('.activity-list').append(data);
					    }else{
			...
			if($settings.scroll == true) {
				...
				$(window).scroll(function() {
					...
					if($(window).scrollTop() + $(window).height() > $this.height() && !busy) {
						...
						$this.find('.loading-bar').html('<img src="images/ajax_loading.gif" alt="Loading">');
