$(function() {
    ...
    singleProductPage.on('click', function(e) {
        if (singleProductPage.hasClass('visible')) {
            // singleProductPage.css("visibility", "none");
            ...
            if (clicked.hasClass('close') || clicked.hasClass('overlay')) {
                ...
                singleProductPage.css("display", "none");
            }
        }
    });
    // These are called on page load
    // Get data about our products from products.json.
    $.getJSON("js/site/products.json", function(data) {
    ...
    $(window).on('hashchange', function() {
        if (window.location.hash != "")render(decodeURI(window.location.hash)); else alert("not changing");
    });
    // Navigation
    function render(url) {
    ...
    function renderSingleProductPage(index, data) {
        ...
        $.ajax({ 
               ...
               dataType:'json',
               data: {"seourl" : index},
               url: 'site/product/get_product_details', // target element(s) to be updated with server response 
               cache : false,
               //check this in firefox browser
               success : function(response){
                content=response;//console.log(response['meta_product_img']); alert(response);
                container.find('h3').text(content['heading']);
                container.find('img').attr('src', '/images/product/'+content['meta_product_img']);
                container.find('p').text(content['added_item_details'][0]['description']);
